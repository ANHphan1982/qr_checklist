...
// ---------------------------------------------------------------------------
// GPS probe chi tiết — thử high-accuracy trước, rồi low-accuracy, đo timing,
// phân biệt nguyên nhân: unsupported / insecure / denied / timeout / hardware.
// ---------------------------------------------------------------------------
const GEO_ERR_NAME = {
  1: "PERMISSION_DENIED",
...
async function probeGps() {
  ...
  // Probe 2: low-accuracy (WiFi/cell fallback) — chỉ thử nếu high-accuracy fail
  let lo = null;
  if (!hi.ok) {
