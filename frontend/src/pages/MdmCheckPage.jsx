...
function useChecks() {
  ...
  const run = useCallback(async () => {
    ...
    // 4. GPS
    set("gps", STATUS.RUNNING, "Đang xin quyền GPS...");
    await new Promise((resolve) => {
      if (!navigator.geolocation) {
        set("gps", STATUS.WARN, "Trình duyệt không hỗ trợ GPS — bỏ qua");
        resolve();
        return;
      }
      navigator.geolocation.getCurrentPosition(
        () => {
          set("gps", STATUS.PASS, "GPS hoạt động bình thường");
          resolve();
        },
        (err) => {
          ...
          set(
            "gps",
            denied ? STATUS.WARN : STATUS.WARN,
            denied
              ? `Quyền GPS bị từ chối — scan vẫn hoạt động nhưng không ghi vị trí. ` +
                `Nếu muốn GPS: MDM → Location Services → Allow`
              : `GPS không khả dụng (code ${err.code})`
          );
          resolve();
        },
        { timeout: 8000 }
...
function CheckRow({ icon, label, result }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3">
      ...
      {detail && (
        <p className="mt-2 ml-10 text-[12px] text-slate-500 dark:text-slate-400 leading-snug">
          {detail}
        </p>
      )}
    </div>
